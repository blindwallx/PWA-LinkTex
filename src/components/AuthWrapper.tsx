...
import OperariosManagement from './OperariosManagement';
import OperarioPanel from './OperarioPanel'; // Importa el nuevo componente
...
const AuthWrapper: React.FC = () => {
            ...
            if (user) {
                ...
                try {
                    ...
                    if (userDocSnap.exists()) {
                        ...
                        setUserStatus(userData.status as string || null);
                            ...
                            retryTimeout = setTimeout(async () => {
                                try {
                                    ...
                                    if (userDocSnapRetry.exists()) {
                                        ...
                                        setUserStatus(userDataRetry.status as string || null);
    ...
    // Aquí no necesitamos construir fullUserName para pasarlo directamente a MainLayout como un prop.
    // MainLayout se encargará de construirlo para su OutletContext.
    ...
    return (
        <>
            ...
            <Routes>
                ...
                {/* Ruta principal que usa MainLayout como layout base */}
                <Route
                    path="/"
                    element={
                        <ProtectedRoute user={currentUser} loading={loading} userStatus={userStatus}>
                            {/* MainLayout recibe todos los props necesarios */}
                            <MainLayout
                                userRole={userRole}
                                companyId={companyId}
                                firstName={firstName}
                ...
                >
                    {/* Rutas anidadas que se renderizan dentro del <Outlet /> de MainLayout */}
                    <Route index element={<Dashboard />} />
                    <Route path="dashboard" element={<Dashboard />} />
                    <Route path="product-management" element={
                        userRole === 'admin' ? <ProductManagement /> : <Navigate to="/dashboard" replace />
                    } />
                    <Route path="profile" element={<Profile />} />
                    <Route path="operarios-management" element={
                        userRole === 'admin' ? <OperariosManagement /> : <Navigate to="/dashboard" replace />
                    } />
                    <Route path="operario-panel" element={
                        userRole === 'operario' ? <OperarioPanel /> : <Navigate to="/dashboard" replace />
                    } />
                </Route>
