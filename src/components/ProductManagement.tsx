...
import ProductFormModal from './ProductFormModal'; // Importamos el nuevo modal

import { FaEdit, FaTrash, FaPlus, FaSearch } from 'react-icons/fa';

// --- Interfaces de Datos (Mantenerlas consistentes o importar desde un archivo compartido) ---
interface Process {
  name: string;
  value: number;
}
...
const ProductManagement: React.FC = () => {
  ...
  const getProductsAndVariants = useCallback(async () => {
    if (!companyId) {
      setError("No se pudo obtener la ID de la empresa. Acceso denegado.");
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (err instanceof Error) {
        ...
        setError("No se pudo cargar los productos. Error desconocido.");
      }
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    if (userRole === 'admin') {
      ...
      setLoading(false);
      setProducts([]);
      setError("No tienes permisos para ver y gestionar productos.");
    }
  ...
  // Ahora, ProductFormModal gestiona su propio `formData` y `handleChange`.
  // Aquí solo necesitamos la función `handleSaveProduct` que `ProductFormModal` llamará.
    ...
    try {
      if (productId) {
        ...
        await updateDoc(productDocRef, {
          ...
          ref: productData.ref, // La referencia no cambia en edición en el modal, pero la actualizamos por consistencia
      ...
      // No reseteamos `newProductForm` aquí, ya que el modal lo hace internamente al cerrarse
      // setNewProductForm({ name: '', ref: '', productionCost: NaN });
      // El modal se encargará de cerrarse si el guardado es exitoso
      await getProductsAndVariants(); // Refrescar la lista de productos
    } catch (err: unknown) {
      if (err instanceof Error) {
        console.error("Error al guardar producto:", err.message);
        setError("Error al guardar producto: " + err.message); // Establece el error en ProductManagement
      } else {
        console.error("Error al guardar producto:", err);
        setError("Error al guardar producto: Error desconocido.");
      }
      // Re-lanza el error para que el modal pueda manejarlo (ej. no cerrarse)
      throw err;
    }
  };
  ...
  const startEditingProduct = (product: Product) => {
    ...
    // Ya no necesitamos setNewProductForm aquí, el modal lo inicializa
  ...
  const deleteProduct = async (productId: string) => {
    setError(null);
    if (!companyId) {
      setError("No se pudo obtener la ID de la empresa. Error al eliminar el producto.");
      return;
    }
  ...
  const handleSaveProcess = useCallback(async (
    ...
    try {
      ...
      if (err instanceof Error) {
        ...
        setError("No se pudo guardar el proceso. " + err.message); // Establecer error en ProductManagement
        ...
        setError("No se pudo guardar el proceso. Error desconocido."); // Establecer error en ProductManagement
  ...
  const handleDeleteProcess = useCallback(async (
    ...
    try {
      ...
      if (err instanceof Error) {
        ...
        setError("No se pudo eliminar el proceso. " + err.message); // Establecer error en ProductManagement
        ...
        setError("No se pudo eliminar el proceso. Error desconocido."); // Establecer error en ProductManagement
  ...
  const handleSaveVariant = useCallback(async (
    ...
    try {
      ...
      if (err instanceof Error) {
        ...
        setError("No se pudo guardar la variante. " + err.message); // Establecer error en ProductManagement
        ...
        setError("No se pudo guardar la variante. Error desconocido."); // Establecer error en ProductManagement
  ...
  const handleDeleteVariant = useCallback(async (productId: string, variantId: string) => {
    ...
    try {
      ...
      if (err instanceof Error) {
        ...
        setError("No se pudo eliminar la variante. " + err.message); // Establecer error en ProductManagement
        ...
        setError("No se pudo eliminar la variante. Error desconocido."); // Establecer error en ProductManagement
  ...
  const closeProductModal = () => {
    ...
    setError(null); // Limpiar errores del padre
  };
  ...
  return (
    <div className="product-management-container">
              ...
              {filteredProducts.map(product => (
                <tr className="product-row" key={product.id}>
                  ...
                  <td className="product-actions-cell" data-label="Acciones">
                    ...
                    <button onClick={() => deleteProduct(product.id!)} className="delete-button icon-button" title="Eliminar Prenda">
                      <FaTrash />
                    </button>
                  </td>
                </tr>
      ...
      <ProductFormModal
        ...
        errorMessage={error} // Pasa el mensaje de error al modal
